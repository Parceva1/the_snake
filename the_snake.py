...
def main():
    """Основной цикл в котором мы реализуем смену направления движения змейки
    и запускаем все отрисовки объектов и фона.
    Так же запускаем методы реализующие передвижение змейки move()
    и обновления экрана update()
    """
    apple = Apple()
    snake = Snake()
